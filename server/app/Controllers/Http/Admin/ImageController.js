...
class ImageController {
    ...
    try {
      ...
      if (!fileJar.files) {
        ...
        if (file.moved()) {
          ...
          images.push(await transform.item(image, Transformer))
  ...
  async show({ params: { id }, response, transform }) {
    return response.send(await transform.item(Image.findOrFail(id), Transformer))
  }
  ...
  async destroy({ params: { id }, response }) {
    ...
    try {
      // let file_path = Helpers.publicPath(`uploads/${image.path}`)

      fs.unlinkSync(Helpers.publicPath(`uploads/${image.path}`))
      await image.delete()
